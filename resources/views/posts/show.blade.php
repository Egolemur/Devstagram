            ...
            @auth
                ...
                @if($post->checkLike(auth()->user()))                    
                    <form method="POST" action="{{ route('posts.likes.destroy', $post) }}"> 
                        ...
                        <div class="my-4">
                            <button type="submit">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="red" viewBox="0 0 24 24" stroke-width="0.5" stroke="currentColor" class="w-6 h-6">
                ...
                @endif                
            @endauth    
            {{-- <a href="{{ route('posts.likes.index', $user) }}">
            ...
            <div class="shadow bg-white p-5 mb-5">
                ...
                <form action="{{ route('comment.store', ['post' => $post, 'user' => $user]) }}" method="POST">
                    ...
                    <div class="mb-5">   
                        ...
                        <textarea id="comentario" name="comentario" type="text" placeholder="Agrega un comentario." class="border p-3 w-full rounded-lg @error('comentario') border-red-500 @enderror">
                        </textarea>    
                        ...
                        @if ($post->comments->count())
                            @foreach ($post->comments as $comment)
                                <div class="p-5 border-gray-300 border-b">
                                    <a class="flex gap-4 items-center" href="{{ route('posts.index', $comment->user) }}">
                                        <img class="w-8 my-4" src="{{ asset('uploads') . '/' . $post->imagen }}" alt="prueba">
                                        <p class="text-lg text-blue-500 font-bold">{{$comment->user->username}}</p>                    
                                    </a>
                            ...
                            @endforeach
                        @else
                            <p class="p-10 text-center"> No hay comentarios aún. Sé el primero en comentar! </p>
                        @endif
                    </div>                    
                </form>
            </div>
